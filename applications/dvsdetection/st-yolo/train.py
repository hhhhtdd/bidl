...
def main(config: DictConfig):
    ...
    # ---------------------
    logger = TensorBoardLogger(save_dir='lightning_logs', name='st_yolo_das')
    ckpt_path = None
    ...
    # ---------------------
    # Callbacks and Misc - 只使用兼容TensorBoard的回调
    # ---------------------
    callbacks = list()
    callbacks.append(get_ckpt_callback(config))
    # 移除 GradFlowLogCallback - 不兼容TensorBoard
    # 移除 viz_callback - 不兼容TensorBoard
    if config.training.lr_scheduler.use:
        callbacks.append(LearningRateMonitor(logging_interval='step'))
    callbacks.append(ModelSummary(max_depth=2))
